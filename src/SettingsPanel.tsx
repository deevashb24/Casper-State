...
export default function SettingsPanel({ settings, onChange, onClose, initialTab, onOpenWiki }: Props) {
  ...
  // ── Full backup / restore (all Casper State data) ──
  ...
  const exportAll = async () => {
    ...
    const payload = { app: 'casperstate', version: 1, exportedAt: new Date().toISOString(), data }
    ...
    a.href = url
    a.download = `casperstate-backup-${new Date().toISOString().slice(0, 10)}${pwd ? '-encrypted' : ''}.json`
    a.click()
  ...
  const importAll = async (file: File) => {
    try {
      ...
      if (payload?.app !== 'casperflow' && payload?.app !== 'casperstate' || !payload.data || typeof payload.data !== 'object') {
        setBackupStatus('Not a Casper State backup file.')
        return
      }
      let n = 0
  ...
  const testAi = async () => {
    ...
    const v = await askAi(
      ...
      'This is a Casper State connection test.',
    )
    const ok = !!v
    let note = ok
  ...
  const testTelegram = async () => {
    ...
    const ok =
      settings.telegramToken && settings.telegramChatId
        ? await sendTelegram(settings.telegramToken, settings.telegramChatId, 'Casper State connection test ✓')
        : false
  ...
  const testDiscord = async () => {
    ...
    const ok = isDiscordWebhook(settings.discordWebhook)
      ? await sendDiscord(settings.discordWebhook, 'Casper State connection test ✓')
      : false
      ...
      <div className="settings-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {tab === 'connections' && openConn === 'casper' && (
          <>
            ...
            <div className="settings-note">
              The key lets Casper State read real on-chain data — wallet balances, the
            ...
            <div className="settings-note">
              ...
              runs the step without Casper State pausing for you; <b>Ask approval</b> = the branch
        ...
        {tab === 'connections' && openConn === 'mcp' && (
          <>
            ...
            <p className="settings-hint">
              Casper State ships a standalone <b>Model Context Protocol</b> server so any MCP-compatible
        ...
        {tab === 'connections' && openConn === 'csprtrade' && (
          <>
            ...
            <p className="settings-hint">
              Casper State's <b>CSPR.trade swap</b> action executes real swaps through the official
              ...
              swaps, liquidity, portfolio) the same way they use Casper State's own MCP — a fully
