...
export const WIKI: WikiSection[] = [
  {
    ...
    articles: [
      ...
      {
        id: 'what-is',
        title: 'What is Casper State?',
        body:
          'Casper State is a visual, no-code canvas to build autonomous AI agents that run on the Casper Network. Think of it as "Zapier for on-chain money": you connect blocks — called actions — to define what your agent watches and what it does, without writing any code.\n\nAn agent is made of three kinds of actions chained together: a trigger (when to run), optional logic (conditions, AI decisions, delays), and one or more actions (swap, send, notify…).',
      ...
      },
      {
        ...
        body:
          'A live agent runs inside this browser tab. Casper State protects it as much as a browser allows:\n\n- Screen wake lock: while the agent is live, Casper State asks the system to keep the screen awake, preventing most laptops from sleeping mid-run.\n- Offline detection: if the connection drops, cycles pause with a clear log entry; the moment you are back online, a catch-up cycle runs automatically.\n- Sleep detection: if the computer did sleep, the agent notices the time gap on wake, logs how many cycles were missed, and resumes immediately.\n\nHard limit: if the computer is fully asleep or off, no browser can keep running. True 24/7 operation requires Casper State Cloud (planned) — your agents run on a server and keep watching the market and sending alerts with your computer off.',
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        body:
          'A blank action where you write your own JavaScript to do anything Casper State doesn’t have a card for. Your code is not just a calculator — it gets a toolkit object called cf that can read live data, call the AI, send notifications, read balances and fetch the web.\n\nYour code returns an object:\n\nreturn { output: "text shown in the log", pass: true, vars: { myValue: 42 } }\n\n- output: what to show in the execution log.\n- pass: true to continue the branch, false to stop it.\n- vars: values you expose to later actions (use them as {{myValue}}).\n\nYour code can be async — use await freely. See "Custom code: the cf toolkit" for the full API and the tutorials below.\n\nSecurity: only run code you wrote or reviewed. When you import a shared workflow that contains code actions, Casper State warns you — always read the code (double-click the card) before running.',
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        title: 'Call contract',
        body:
          'Calls any entry point of any deployed Casper contract with JSON arguments — the universal action that lets your agent interact with contracts Casper State doesn’t have a dedicated card for. Set the contract hash, the entry point name (e.g. transfer, mint), and the arguments.',
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        id: 'webhook',
        title: 'HTTP webhook',
        body: 'Sends an HTTP POST with the run payload to any URL — connect Casper State to your own systems.',
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        body:
          'The "Connect wallet" button in the top bar links Casper State to the Casper Wallet browser extension (get it at casperwallet.io).\n\nOnce connected, the button shows your account and live balance (when a CSPR.cloud key is set), and your wallet automatically becomes the watched account for on-chain triggers if none was set. Click again to disconnect. Switching accounts in the extension is detected automatically.\n\nYour keys never leave the extension — Casper State only sees your public key. Transaction signing for real actions uses the wallet prompt (or a dedicated agent key in autonomous mode — see "Signing").',
